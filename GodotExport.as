package {
	...
	public class GodotExport extends Sprite {
		...
		private var clipIndexPaths:Dictionary = new Dictionary();
		...
		private var bitmapDataOrder:Array = new Array();
			...
			for each(var _sd1 : SceneData in SceneData.allSceneData) {
				for (var f:int = _sd1.startFrame; f <= _sd1.endFrame; f++) 
				{
					...
					for each (var _clipName:String in listAnimatedClip) 
					{
						...
						var _clip:DisplayObject = detector.findClipByName(_root, _clipName);
						if (_clip == null && isAutoGeneratedName(_clipName)) {
							_clip = findClipByIndexPath(clipIndexPaths[_clipName], _clipName);
						}
		...
		private function generateAtlas():void {
			// On utilise bitmapDataOrder (ordre d'export déterministe) au lieu
			// d'itérer le Dictionary, dont l'ordre dépend des hashs des clés
			// (et donc des suffixes aléatoires) et changeait à chaque export.
			if (bitmapDataOrder.length == 0) {
				return;
			}
			...
			for (var oi:int = 0; oi < bitmapDataOrder.length; oi++) {
				var id:String = bitmapDataOrder[oi];
		...
		private function parseDisplayObject(obj:DisplayObject, _parent:*, _index : String ='', _parent_path = '.'):void {
			...
			if (!(nodeName in clipTypes)) {
				...
				clipIndexPaths[nodeName] = _index;
			}
		...
		// Détecte les noms d'instance auto-générés par Flash (ex: "instance57").
		// Ces noms ne sont pas stockés dans le SWF et sont réattribués au runtime,
		// donc ils peuvent changer d'une scène à l'autre.
		private function isAutoGeneratedName(_name:String):Boolean
		{
			if (_name == null) return false;
			return /^instance\d+$/i.test(_name);
		}

		// Résout un clip par son chemin d'index structurel (ex: "3-0").
		// Contrairement au nom, ce chemin reste valable d'une scène à l'autre
		// même quand Flash réattribue les noms d'instance auto-générés.
		// Pour un clip imbriqué, on vérifie que l'ancêtre racine porte bien le
		// nom attendu afin d'éviter de résoudre un objet au hasard si le parent
		// est absent de la scène.
		private function findClipByIndexPath(_indexPath:*, _clipName:String):DisplayObject
		{
			if (_indexPath == null || _indexPath == "") return null;

			var _pathStr:String = clipPaths[_clipName] != null ? String(clipPaths[_clipName]) : "";
			var _expectedRoot:String = (_pathStr.indexOf("/") != -1) ? _pathStr.split("/")[0] : null;

			var parts:Array = String(_indexPath).split("-");
			var container:DisplayObjectContainer = _root;

			for (var i:int = 0; i < parts.length; i++) {
				if (container == null) return null;

				var idx:int = parseInt(parts[i]);
				if (isNaN(idx) || idx < 0 || idx >= container.numChildren) return null;

				var child:DisplayObject = container.getChildAt(idx);

				if (i == 0 && _expectedRoot != null && child.name != _expectedRoot) {
					return null;
				}
		...
		private function insertAnimationDatas() 
		{
			...
			{
				trace(label.name);
				if (label.name.toUpperCase().indexOf('LOOP') != -1)
				{
					hasLoopLabel = true;
					break;
				}
			...
			{
				if(_label.name.toUpperCase().indexOf('LOOP') != -1) continue;
